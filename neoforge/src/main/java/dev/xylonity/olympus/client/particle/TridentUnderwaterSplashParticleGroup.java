...
public final class TridentUnderwaterSplashParticleGroup {
    ...
    public static final ParticleRenderType TYPE = new ParticleRenderType() {
        ...
        @Override
        public BufferBuilder begin(final Tesselator tesselator, final TextureManager textureManager) {
            RENDER_TYPE.setupRenderState();
            return tesselator.begin(RENDER_TYPE.mode(), RENDER_TYPE.format());
        }
    ...
    private static void addVertex(final VertexConsumer buffer, final Vec3 position, final TridentUnderwaterSplashParticle.RenderSnapshot snapshot) {
        buffer.addVertex((float) position.x, (float) position.y, (float) position.z)
                .setColor(snapshot.red(), snapshot.green(), snapshot.blue(), snapshot.alpha());
    }
