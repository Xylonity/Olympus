...
public final class OlympusClientEvents {
    ...
    public static void clientSetup(final FMLClientSetupEvent event) {
        event.enqueueWork(() -> {
            ...
            ItemProperties.register(OlympusItems.BOW_OF_ARTEMIS.get(), ResourceLocation.withDefaultNamespace("pulling"),
                    (stack, level, entity, seed) -> entity != null && entity.isUsingItem() && entity.getUseItem() == stack ? 1.0F : 0.0F);
            ItemProperties.register(OlympusItems.BOW_OF_ARTEMIS.get(), ResourceLocation.withDefaultNamespace("pull"),
    ...
    public static void registerGuiLayers(final RegisterGuiLayersEvent event) {
        event.registerAbove(VanillaGuiLayers.CAMERA_OVERLAYS, Olympus.of("hades_screen_filter"),
                (guiGraphics, partialTick) -> {
                    final Minecraft minecraft = Minecraft.getInstance();
                    if (minecraft.player == null || !minecraft.player.hasEffect(BuiltInRegistries.MOB_EFFECT.wrapAsHolder(OlympusMobEffects.INVISIBILITY_OF_HADES.get()))) {
                        return;
                    }
    ...
    public static final class ForgeEvents {
        ...
        @SubscribeEvent
        public static void onHermesClientTick(final ClientTickEvent.Pre event) {
            final Minecraft minecraft = Minecraft.getInstance();
        ...
        public static void onClientTick(final ClientTickEvent.Post event) {
            ...
            final boolean stunned = minecraft.getCameraEntity() instanceof LivingEntity livingEntity
                    && livingEntity.hasEffect(BuiltInRegistries.MOB_EFFECT.wrapAsHolder(OlympusMobEffects.LIGHTNING_STUN.get()));
