...
public final class InvisibilityOfHadesEffect extends MobEffect {
    ...
    public static boolean preventsTargeting(final LivingEntity target) {
        return target.hasEffect(BuiltInRegistries.MOB_EFFECT.wrapAsHolder(OlympusMobEffects.INVISIBILITY_OF_HADES.get()));
    }

    public static void shortenAfterAttack(final ServerPlayer player) {
        final MobEffectInstance effect = player.getEffect(BuiltInRegistries.MOB_EFFECT.wrapAsHolder(OlympusMobEffects.INVISIBILITY_OF_HADES.get()));
        if (effect != null) {
