...
public final class SpearOfAresEntity extends ThrownTrident implements KnightLibAnimatable {
    ...
    protected void defineSynchedData(final SynchedEntityData.Builder builder) {
        ...
        builder.define(COLLISION_START_TICK, -1L);
    }
    ...
    protected void onHitEntity(final EntityHitResult hitResult) {
        ...
        final float damage = EnchantmentHelper.modifyDamage(serverLevel, getSpearStack(), target, damageSource, configuredDamage);
        if (target.hurt(damageSource, damage)) {
            if (currentOwner instanceof LivingEntity livingOwner) {
                livingOwner.setLastHurtMob(target);
            }

            EnchantmentHelper.doPostAttackEffectsWithItemSource(serverLevel, target, damageSource, getSpearStack());
    ...
    public void readAdditionalSaveData(final CompoundTag input) {
        ...
        if (input.contains("Weapon", 10)) {
            spearStack = ItemStack.parseOptional(registryAccess(), input.getCompound("Weapon"));
        }
    ...
    public void addAdditionalSaveData(final CompoundTag output) {
        ...
        output.put("Weapon", spearStack.save(registryAccess()));
    }
